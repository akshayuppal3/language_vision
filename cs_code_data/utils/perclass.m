...
%Train a -1/1 SVM for each class
nim=size(label,1);
...
for i=1:size(label,2)
	for foldid=1:nfolds
		...
		model_crossval{i,foldid}=train(label(fold_ind_inv,i),sparse(data(fold_ind_inv,:)),['-q -c ' num2str(c)]);
		if length(model_crossval{i,foldid}.Label)==2
			[~,~,scores_cval]=predict(label(fold_ind,i),sparse(data(fold_ind,:)),model_crossval{i,foldid},'-q');
			[acc_crossval(i,foldid),random_crossval(i,foldid)]=precision(scores_cval*model_crossval{i,foldid}.Label(1),label(fold_ind,i));
		else
